...
import { createClip, filterUnseenUrls, listThemes, logIngest } from './db';
...
async function fetchCandidates(themes) {
...
export async function runIngest({ dryRun = false, limit = MAX_PER_RUN } = {}) {
  ...
  const themes = (await listThemes()).map((t) => t.text);
  const { themedCandidates, generalCandidates, feedsOk, feedsFailed } =
    await fetchCandidates(themes);
  ...
  // Themed articles lead but never take every slot, so general news
  // keeps flowing even when themes are set.
  const cap = Math.max(1, limit);
  const themedCap = themes.length ? Math.ceil(cap * 0.6) : 0;
  const themedQueue = themedCandidates.filter((c) => unseenSet.has(c.url)).slice(0, themedCap);
  const generalQueue = generalCandidates
    .filter((c) => unseenSet.has(c.url))
    .slice(0, cap - themedQueue.length);
