...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      `;
      // Editor-set topics that steer what the wirebot searches for.
      await sql`
...
// Themes older than maxAgeDays silently stop steering the bot; editors can
// also delete them outright.
export async function listThemes({ maxAgeDays = 7 } = {}) {
  if (dbUrl) {
    await ensureSchema();
    return sql`
      SELECT * FROM themes
      WHERE created_at > NOW() - make_interval(days => ${maxAgeDays})
...
export async function createTheme({ text, addedBy = null }) {
  if (dbUrl) {
    await ensureSchema();
    const rows = await sql`
      INSERT INTO themes (text, created_by) VALUES (${text}, ${addedBy}) RETURNING *
    `;
    return rows[0];
  }
  const store = memStore();
  store.themes = store.themes || [];
  store.nextThemeId = store.nextThemeId || 1;
  const theme = {
...
export async function deleteTheme(id) {
  if (dbUrl) {
    await ensureSchema();
    const rows = await sql`DELETE FROM themes WHERE id = ${id} RETURNING id`;
    return rows.length > 0;
  }
  const store = memStore();
  store.themes = store.themes || [];
